...
Pi = label_to_membership(y_train).to(device)
...
with torch.no_grad():
    ...
    total_r = 0
    for j in range(n_class):
        ZZT = Z_train.T @ Pi[:, j].diag() @ Z_train
        U, S, _ = torch.linalg.svd(ZZT)
        r = 1
        error = float('inf')
        while True:
            error = torch.norm(ZZT - U[:,:r]@torch.diag(S[:r])@U[:,:r].T)**2
            if error <= args.tol:
                break
            r += 1
        print(r)
        ...
        init_A.append(S[:r])
print('r: %d'%total_r)
U = torch.cat(init_U, dim=1)
A = torch.zeros((n_class, total_r))
r = 0
for i in range(n_class):
    A[i] = torch.tensor([0]*r + init_A[i].tolist() + [0]*(total_r -r -init_A[i].shape[0]))
    r += init_A[i].shape[0]
