...
def save_array(model_dir, array, name, folder=None):
    save_dir = os.path.join(model_dir, 'arrays')
    if folder is not None:
        save_dir = os.path.join(save_dir, folder)
    os.makedirs(save_dir, exist_ok=True)
    np.save(os.path.join(save_dir, f"{name}.npy"), array)
    print('Array saved to:', os.path.join(save_dir, f"{name}.npy"))

def create_csv(model_dir, filename, headers):
    """Create .csv file with filename in model_dir, with headers as the first line 
    of the csv. """
    csv_path = os.path.join(model_dir, 'csv', f'{filename}.csv')
    os.makedirs(os.path.join(model_dir, 'csv'), exist_ok=True)
